...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=v2x \
...
build-docker:
	...
	$(DOCKER) create --name v2x -t -i ${DOCKER_IMAGE}:latest v2x
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp v2x:/usr/bin/v2xd ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/v2x:Z v2x/node "./v2xd testnet init-files --v 4 -o /v2x --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\v2xd:/evmos\Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node1\v2xd:/evmos\Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node2\v2xd:/evmos\Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node3\v2xd:/evmos\Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/build/node0/v2xd:/evmos:Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node1/v2xd:/evmos:Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node2/v2xd:/evmos:Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node3/v2xd:/evmos:Z evmos/node "./v2xd tendermint unsafe-reset-all --home=/evmos"
endif
