...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=volley \
...
build-docker:
	...
	$(DOCKER) rm volley || true
	# create a new container from the latest image
	$(DOCKER) create --name volley -t -i ${DOCKER_IMAGE}:latest volley
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp volley:/usr/bin/volleyd ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/volley:Z volley/node "./volleyd testnet init-files --v 4 -o /volley --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
